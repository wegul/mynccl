...
ncclResult_t wrap_ibv_fork_init(void) {
  IBV_INT_CHECK_RET_ERRNO(ibvSymbols, ibv_internal_fork_init,
                          ibv_internal_fork_init(), 0, "ibv_fork_init");
}

ncclResult_t wrap_ibv_get_device_list(struct ibv_device ***ret,
                                      int *num_devices) {
  struct ibv_device **list =
      ibvSymbols.ibv_internal_get_device_list(num_devices);
  if (list == NULL)
    return ncclSystemError;
  *ret = list;
  return ncclSuccess;
...
ncclResult_t wrap_ibv_close_device(struct ibv_context *context) {
  IBV_INT_CHECK_RET_ERRNO(ibvSymbols, ibv_internal_close_device,
                          ibv_internal_close_device(context), 0,
                          "ibv_close_device");
}

ncclResult_t wrap_ibv_get_async_event(struct ibv_context *context,
                                      struct ibv_async_event *event) {
  IBV_INT_CHECK_RET_ERRNO(ibvSymbols, ibv_internal_get_async_event,
                          ibv_internal_get_async_event(context, event), 0,
                          "ibv_get_async_event");
}
...
ncclResult_t wrap_ibv_alloc_pd(struct ibv_pd **ret,
  ...
  IBV_PTR_CHECK(ibvSymbols, ibv_internal_alloc_pd,
                ibv_internal_alloc_pd(context), *ret, NULL, "ibv_alloc_pd");
}
...
ncclResult_t wrap_ibv_create_qp(struct ibv_qp **ret, struct ibv_pd *pd,
  ...
  IBV_PTR_CHECK(ibvSymbols, ibv_internal_create_qp,
...
ncclResult_t wrap_ibv_create_srq(struct ibv_srq **ret, struct ibv_pd *pd,
  ...
  IBV_PTR_CHECK(ibvSymbols, ibv_internal_create_srq,
