...
# host pf0_if pf1_if hca_sout hca_sup cpu_set
entry=$(awk -v h="${host}" '!/^[[:space:]]*#/ && NF && $1==h {print; exit}' "${HOST_MAP}")
[[ -n "${entry}" ]] || { echo "ERROR: no host-map entry for ${host} in ${HOST_MAP}" >&2; exit 2; }

read -r map_host pf0_if pf1_if hca_sout hca_sup cpu_core <<< "${entry}"
[[ -n "${pf0_if}" && -n "${pf1_if}" && -n "${hca_sout}" && -n "${hca_sup}" && -n "${cpu_core}" ]] || {
  echo "ERROR: bad host-map row for ${host}: ${entry}" >&2
  exit 2
}

export MPIB_OOB_IF="${MPIB_OOB_IF:-${pf0_if}}"
...
if [[ "${WRAPPER_VERBOSE:-0}" == "1" ]]; then
  echo "[${host}] pf0=${pf0_if}->${MPIB_HCA_SOUT} pf1=${pf1_if}->${MPIB_HCA_SUP} cpu=${cpu_core} NCCL_IB_HCA=${NCCL_IB_HCA}" >&2
fi
